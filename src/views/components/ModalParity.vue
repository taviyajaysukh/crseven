    ...
    methods: {
        ...
        async confirm() {
            if (this.total >= this.userBalance) {
                ...
                let perityPeriod = this.currentPeriod;
                let usermobile = localStorage.getItem('session_user') || ''
                let peritytotaltamount = this.total
                let joinsection = this.number
                let amount = this.amount
                let qty = this.count
                await axios
                    .post('http://localhost:3000/users/addPerity', { "perityperiod": perityPeriod, "usermobile": usermobile,"joinsection":joinsection, "amount": amount, "qty": qty, "peritytotaltamount": peritytotaltamount })
                    .then(res => this.peritydata = res?.data)
                    if(this.peritydata?.status == 'success'){
                        this.toastfunction(this.peritydata?.message, 3000)
                        this.getSessionamount(this.peritydata?.balance)
                        return modalController.dismiss(null, 'cancel');
                    }else{
                        this.toastfunction(this.peritydata?.message, 3000)
                        return modalController.dismiss(null, 'cancel');
