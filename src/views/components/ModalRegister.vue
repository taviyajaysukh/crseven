<template>
    <ion-header>
        <ion-toolbar>
            <ion-title>Register Form</ion-title>
            ...
            <ion-row>
                <ion-col>
                    <div class="input_box">
                        ...
                        <ion-input type="number" placeholder="Mobile Number" v-model="mobile"></ion-input>
                    </div>
                </ion-col>
            </ion-row>
            <ion-row>
    ...
    methods: {
        ...
        async register() {
            ...
            if (this.mobile === "" || mobilepattern !== 0) {
                ...
                this.isvalid = false;
            } else if (this.verification_code === "" || this.verification_code.length <= 5) {
                this.toastfunction("Please enter valid verification code with 5 greater than size", 3000);
                this.isvalid = false;
            ...
            if (this.isvalid === true) {
                await axios
            .post('http://localhost:3000/users/userRegister', {mobile:this.mobile,password:this.password,verification_code:this.verification_code,recommendation_code:this.recommendation_code})
            .then(res => this.userReg = res?.data)
                if(this.userReg?.status || this.userReg.message !=''){
                    if(this.userReg?.status =='success'){
                        this.mobile = ''
                        this.password = ''
                    ...
                    this.toastfunction(this.userReg?.message, 1000);  
                }
            }
        },
        async sendOtp() {
            ...
            if (this.isvalid) {
                await axios
            .post('http://localhost:3000/users/sentOtp', {mobile:this.mobile})
            .then(res => this.otpMessage = res?.data?.message)
            if(this.otpMessage !=''){
                this.toastfunction(this.otpMessage,1000)
            }
            } else {
